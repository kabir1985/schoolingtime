...
<main id="main" class="my-5">
  ...
  <div class="col-lg-9 entries">
      ...
      <div class="col-md-7">
        <select class="form-select" name="exam_name_id" id="exam_name_id" aria-label="Default select example">
          ...
          foreach ($exam_setup_info as $row) {
            ...
            $exam_subject_course_id = $row->exam_subject_course_id;
            ////////////////////Course Name from Teacher Course Table/////////////////////
            $db = \Config\Database::connect();
            $query1   = $db->query("SELECT * FROM teacher_course 
                                Where course_id = '$exam_subject_course_id'");
            $results1 = $query1->getResult();
            foreach ($results1 as $row1) {
              $course_name = $row1->coures_title;
            }
            /////////////////////////////////////////////////////////////////
          ?>
            <option data-course_id=<?php echo $exam_setup_id; ?>><?php echo  $course_name . "--->" . $exam_name; ?></option>
          <?php
          }
          ?>
        </select>
    ...
    <div class="row mt-2">
      ...
      <div class="col-md-7">
        <select class="form-select" name="question_set_name" id="question_set_name" aria-label="Default select example">
          ...
          foreach ($results as $row) {
            ...
            /////////////////////////////////////////////////////////////////
          ?>
            <option data-question_set_id=<?php echo $question_set_id; ?>><?php echo $question_set_title; ?></option>
          <?php
          }
          ?>
        </select>
    ...
    <div class="row">
      ...
      <button type="button" class="btn btn-primary click_checkbox" style="background-color: #465FAB !important;">Submit</button>
    </div>
    ...
    $('.form-select').on("change", function() {
      ...
      var targetTable = $('#questionDataTable').DataTable({
        ...
        columnDefs: [
          ...
          {
            render: (data, type, row) => '<input  ' + ((data == '1') ? 'checked' : '') + ' type="checkbox"  value="' + row.question_bank_id + '" />',
            targets: 2
          },
        ]
      });
